...
from pipeline_config import ENABLED_STAGES, GCUT, NEURON_IDS, NNUNET, PATHS, PRUNING, RUN_MODE, SOMA, VISUALIZATION
...
def run_stage_windowed(neuron_ids: list[int | str], stage_handlers: list[tuple[str, Any]], meta_df: pd.DataFrame | None, max_inflight: int) -> None:
    """Run a rolling stage window without parallel execution.

    A neuron enters stage-1 until ``max_inflight`` are active. Then downstream
    stages consume ready neurons one-by-one; each completion frees a slot for a
    new stage-1 neuron.
    """
    if max_inflight <= 0:
        raise ValueError(f"RUN_MODE.max_inflight must be > 0, got {max_inflight}")

    inflight: list[int | str] = []
    stage_index: dict[int | str, int] = {}
    next_neuron_idx = 0
    total_stages = len(stage_handlers)

    while next_neuron_idx < len(neuron_ids) or inflight:
        while next_neuron_idx < len(neuron_ids) and len(inflight) < max_inflight:
            nid = neuron_ids[next_neuron_idx]
            next_neuron_idx += 1
            inflight.append(nid)
            stage_index[nid] = 0
        ...
        for nid in list(inflight):
            current_stage = stage_index[nid]
            if current_stage >= total_stages:
                inflight.remove(nid)
                stage_index.pop(nid, None)
                progressed = True
                continue

            stage_name, stage_func = stage_handlers[current_stage]
            print(f"\n========== [window] neuron {nid} stage {current_stage + 1}/{total_stages}: {stage_name} ==========")
            stage_func(nid)
            stage_index[nid] = current_stage + 1
            progressed = True

            # One stage-step per scheduler iteration keeps the pipeline interleaved.
            break
...
def main() -> None:
    ...
    enabled_stage_handlers: list[tuple[str, Any]] = [
        (name, func) for name, func in stages if ENABLED_STAGES.get(name, True)
    ]
    if ENABLED_STAGES.get("gcut", True):
        enabled_stage_handlers.append(("gcut", lambda neuron_id: run_gcut(neuron_id, meta_df)))
    if ENABLED_STAGES.get("prune", True):
        enabled_stage_handlers.append(("prune", lambda neuron_id: run_prune(neuron_id, meta_df)))

    mode = str(RUN_MODE.get("mode", "per_neuron")).strip().lower()
    max_inflight = int(RUN_MODE.get("max_inflight", 20))

    if mode == "per_neuron":
        for neuron_id in NEURON_IDS:
            print(f"\n========== Processing neuron {neuron_id} ==========")
            for stage_name, stage_func in enabled_stage_handlers:
                print(f"\n--- {stage_name} ---")
                stage_func(neuron_id)
    elif mode == "staged_window":
        run_stage_windowed(NEURON_IDS, enabled_stage_handlers, meta_df, max_inflight=max_inflight)
    else:
        raise ValueError(f"Unsupported RUN_MODE.mode: {mode}. Use 'per_neuron' or 'staged_window'.")

    print("\nAll requested neurons have been processed.")

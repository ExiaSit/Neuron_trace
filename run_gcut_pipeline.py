...
try:
    ...
    print("警告: 找不到 neuroutils 模块，请确保环境配置正确。")
...
def remove_fake_roots(tree):
    """
    检查树结构中是否有多个根节点，如果有，检查它们是否有子节点。
    如果没有子节点，则认为是假根节点（孤立点）并将其移除。
    返回: (清理后的树, 移除的假根节点数量)
    """
    # 找到所有的根节点
    roots = [node for node in tree if node[6] == -1]
    
    # 如果只有一个或没有根节点，不需要进一步清理
    if len(roots) <= 1:
        return tree, 0
        
    # 收集树中所有被作为 parent_id 引用的节点 ID (即有子节点的节点)
    ...
    for node in tree:
        idx, type_, x, y, z, r, p = node
        # 如果当前节点是根节点，且没有作为父节点出现在集合中，则它是孤立点
...
# 4. 全新综合可视化绘图模块
# =========================================================================
def save_comprehensive_summary(img, binary_img, dist_transform, gsdt_mask, soma_coords, 
                               app2_tree, split_trees, target_soma_id, 
                               save_path, title_prefix=""):
    """
    生成 2x4 的网格图，将所有的预处理、App2结果、GCut结果集中在一张大图中。
    """
    if img.ndim == 3:
        ...
        img_show, binary_show, dist_show, mask_show = img, binary_img, dist_transform, gsdt_mask
    ...
    im3 = ax[2].imshow(dist_show, cmap='magma')
    ax[2].set_title("3. GSDT Distance Map", fontsize=16)
    ax[2].axis('off')

    ax[3].imshow(mask_show, cmap='gray')
    ax[3].set_title(f"4. GSDT > X Mask & Soma Centers (N={len(soma_coords)})", fontsize=16)
    ax[3].axis('off')
    for coord in soma_coords:
        if img.ndim == 3:
            y, x = coord[1], coord[2]
            ...
            y, x = coord[0], coord[1]
        ax[3].plot(x, y, 'r*', markersize=12, markeredgecolor='white')
    ...
    # 图 5：Original App2 Trace Overlay
    ax[4].imshow(img_show, cmap='gray')
    ax[4].set_title("5. Original App2 Trace (Raw)", fontsize=16)
    ax[4].axis('off')
    if app2_tree and len(app2_tree) > 0:
        ...
        for node in app2_tree:
            ...
            if p_id in node_dict:
                ...
                y1 = img_height - node[3]
                y2 = img_height - p_node[3]
                ax[4].plot([node[2], p_node[2]], [y1, y2], color='cyan', linewidth=1.5, alpha=0.6)
    else:
        ax[4].text(0.5, 0.5, 'No App2 Trace Found', color='white', ha='center', va='center', fontsize=16, transform=ax[4].transAxes)

    # 图 6：G-Cut All Segmented Neurons
    ax[5].imshow(img_show, cmap='gray')
    ax[5].set_title("6. G-Cut Segmented (All Context)", fontsize=16)
    ax[5].axis('off')
    if split_trees:
        for sid, tree in split_trees.items():
            ...
            for node in tree:
                ...
                if p_id in node_dict:
                    ...
                    y1 = img_height - node[3]
                    y2 = img_height - p_node[3]
                    ax[5].plot([node[2], p_node[2]], [y1, y2], color=color, linewidth=1.5, alpha=0.8)
            soma_node = node_dict.get(sid)
            if soma_node:
                soma_y = img_height - soma_node[3]
                ax[5].plot(soma_node[2], soma_y, marker='*', color=color, markersize=15, markeredgecolor='white')
    else:
        ax[5].text(0.5, 0.5, 'No G-Cut Results\n(Skipped/Empty)', color='white', ha='center', va='center', fontsize=16, transform=ax[5].transAxes)

    # 图 7：G-Cut Target Neuron Only
    ax[6].imshow(img_show, cmap='gray')
    ax[6].axis('off')
    target_title = f"7. Target Neuron (Soma ID: {target_soma_id})" if target_soma_id is not None else "7. Target Neuron Not Extracted"
    ax[6].set_title(target_title, fontsize=16)
    if target_soma_id is not None and target_soma_id in split_trees:
        ...
        for node in tree:
            ...
            if p_id in node_dict:
                ...
                y1 = img_height - node[3]
                y2 = img_height - p_node[3]
                ax[6].plot([node[2], p_node[2]], [y1, y2], color=color, linewidth=1.5, alpha=0.8)
        soma_node = node_dict.get(target_soma_id)
        if soma_node:
            soma_y = img_height - soma_node[3]
            ax[6].plot(soma_node[2], soma_y, marker='*', color=color, markersize=15, markeredgecolor='white')
    else:
        ax[6].text(0.5, 0.5, 'No Target Match', color='white', ha='center', va='center', fontsize=16, transform=ax[6].transAxes)
...
# 5. 主流水线流程
# =========================================================================
def process_image_to_gcut(img_path, mask_path, swc_path, vis_output_dir, target_coord=None, percentile_value=50.0, gsdt_threshold=5.0):
    ...
    NMS_RADIUS = 15.0 
    soma_coords = apply_nms(raw_soma_coords, raw_gsdt_scores, nms_radius=NMS_RADIUS)
    ...
    save_comprehensive_summary(
        img, binary_img, dist_transform, gsdt_mask, soma_coords,
        app2_tree, split_trees, target_soma_id,
        combined_vis_path, title_prefix=img_path.name
    )
...
def worker_task(args):
    """多进程的任务包装器：将日志写入下放到这里，实现真正的实时落盘"""
    img_path, mask_path, swc_path, vis_dir, pct_val, gsdt_thr, error_log_path = args
    try:
        neuron_id = extract_id(img_path.name)
        ...
        res, log_msg = process_image_to_gcut(img_path, mask_path, swc_path, vis_dir, csv_target_coord, pct_val, gsdt_thr)
...
if __name__ == "__main__":
    ...
    for original_img_file in files_to_process:
        # existing_targets = list(vis_dir.glob(f"{original_img_file.stem}_gcut_soma_*_TARGET.swc"))

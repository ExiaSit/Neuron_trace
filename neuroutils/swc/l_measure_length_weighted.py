    ...
    for id in tqdm(common_id_list):
        ...
        for feature in pred_row.columns:
            if feature == 'id':
                continue
            print(id, feature)
            comp_lm_df.loc[id, feature+"_gt"] = gt_row[feature].values[0]
            comp_lm_df.loc[id, feature+"_pred"] = pred_row[feature].values[0]
            comp_lm_df.loc[id, feature+"_ratio"] = pred_row[feature].values[0] / gt_row[feature].values[0]

    comp_lm_df = comp_lm_df.reset_index(drop=True)
    return comp_lm_df

def plot_comp_l_measure_df(comp_lm_df, todo_list):
    for col in comp_lm_df.columns:
        comp_lm_df[col] = pd.to_numeric(comp_lm_df[col], errors='coerce')
    ...
    # print(position)
    # print(comp_lm_df, comp_lm_df.dtypes)
    ...
    plt.xticks(position, [feature for feature in comp_lm_df.columns if feature != 'id'],
...
if __name__ == '__main__':
    ...
    result_comp_lm_df_file = "//data2/kfchen/tracing_ws/14k_raw_img_data/crop100/comp_lm_df_crop100_1209.csv"
    auto_swc_dir = "/data2/kfchen/tracing_ws/14k_raw_img_data/crop100/auto8.4k_0510_resample1um_mergedBranches0712_crop100"
    manual_swc_dir = "/home/lyf/Research/publication/humain10k/HumanMorphoMap/h01-guided-reconstruction/data/manual_resampled1um_crop100"


    auto_lm_df = l_measure_swc_dir(auto_swc_dir, img_dir, todo_list)
    manual_lm_df = l_measure_swc_dir(manual_swc_dir, img_dir, todo_list)
    print(auto_lm_df.shape, manual_lm_df.shape)
    ...
    plot_comp_l_measure_df(comp_lm_df, todo_list)

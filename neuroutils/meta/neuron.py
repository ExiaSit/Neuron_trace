...
def create_csv(todo_neuron_ids):
    
    full_df = pd.read_csv(NEURON_META_INFO_PATH, encoding="utf-8")
    for neuron_id in todo_neuron_ids:
        local_path = os.path.join(NEURON_META_DIR, f"{neuron_id}.csv")
...
if __name__ == "__main__":
    ...
    todo_neuron_ids = [i for i in range(120000, 120001)]
    create_csv(todo_neuron_ids)
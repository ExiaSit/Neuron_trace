...
RUN_MODE = {
    # "per_neuron": run full pipeline for one neuron, then next (current behavior).
    # "staged_window": run stage-by-stage with a rolling window.
    "mode": "per_neuron",

    # Used only when mode == "staged_window".
    # Example: 20 means stage-1 fills up to 20 ready neurons, then stage-2 starts.
    "max_inflight": 20,
}
